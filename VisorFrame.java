...
public class VisorFrame extends JFrame {
    ...
    private JPanel crearStatusBar() {
        ...
        // ✅ Actualizado para reflejar los gestos reales
        JLabel hint = new JLabel("  Gestos: MOVER (5 dedos) → Rotar  |  2 manos juntas/alejadas → Zoom");
    ...
    static class Visor3DPanel extends JPanel {
        ...
        private static final double LERP=0.15, ROT_H=180, ROT_V=130, ZOOM_PASO=0.08;
        ...
        private static final double LX=0.408, LY=0.816, LZ=0.408;

        private int  timerDelay = 33;
        private long tUltimo=0, acum=0;
        ...
        private void tick() {
            ...
            if (Math.abs(dy)>0.005 || Math.abs(dp)>0.005 || Math.abs(dz)>0.0005) {
                ...
                if (tUltimo > 0) {
                    acum += now - tUltimo;
                    if (++nFrames >= N) { ajustarFPS(acum/nFrames); acum=0; nFrames=0; }
                }
                tUltimo = now;
            }
        }
        ...
        // ✅ procesarComando actualizado: MOVER + ZOOM +++/---
        public void procesarComando(String cmd) {
            ...
            switch (p[0]) {
                ...
                case "ZOOM":
                    if (p.length == 2) {
                        switch (p[1]) {
                            case "+++": zoomTarget = Math.min(zoomTarget + ZOOM_PASO * 3, 3.0); break;
                            case "---": zoomTarget = Math.max(zoomTarget - ZOOM_PASO * 3, 0.3); break;
                            // compatibilidad con signos simples por si acaso
                            case "+":   zoomTarget = Math.min(zoomTarget + ZOOM_PASO,     3.0); break;
                            case "-":   zoomTarget = Math.max(zoomTarget - ZOOM_PASO,     0.3); break;
                        }
                    }
                    break;

                // ── Resetear tracking al cambiar de gesto ─────────────────
                case "NONE":
                case "BORRADOR":
                case "BORRAR_TODO":
                case "CAMBIAR_COLOR":
        ...
        private void render(Graphics2D g2, int W, int H) {
            ...
            for (int i = 0; i < nV; i++) {
                ...
                double y2 = v[1]*cosP - z1*sinP;
                double z2 = v[1]*sinP + z1*cosP;
                double zc = z2 + camZ;
                if (zc < 0.01) zc = 0.01;
                sx[i] = cxW + x1*fov/zc;
                sy[i] = cyH - y2*fov/zc;
            ...
            for (int i = 0; i < nC; i++) {
                int[] c = modelo.caras[i];
                double s = 0;
                for (int vi : c) s += sz[vi];
                zC[i] = s / c.length;
            }
